...
public partial class ZyzgList : System.Web.UI.Page
{
    #region 通用过程
    ...
    protected void ObjGrid_ItemRowBound(object sender, GridViewRowEventArgs e)
    {
        ...
        ImgInfo.Attributes["onclick"] = "javascript:if (window.confirm('你确定要修改选择的记录吗？')) { window.parent.frames['fmBottom'].navigate('ZyzgEdit.aspx?id=@id');} else { return false; }".Replace("@id", id);

        Image ImgDel = (Image)e.Row.FindControl("ImgDel");
        ImgDel.Attributes["onclick"] = "javascript:if (window.confirm('你确定要删除选择的记录吗？')) { $find('PopUp').show();window.navigate('ZyzgList.aspx?delid=@delid'); } else { return false; }".Replace("@delid", id);
    ...
    private void DeleteItem(string id)
    {
        try {
            ResumeInfoDataContext db = new ResumeInfoDataContext();
            Zhiyzg item = db.Zhiyzg.Single(o => o.PKID == int.Parse(id));
            db.Zhiyzg.DeleteOnSubmit(item);
            db.SubmitChanges();
    ...
    {
        WebHelper.WriteScript("window.navigate('ZyzgList.aspx');", ClientScript);
    }
}