...
public partial class FindResult : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack) {
            ...
            string backurl = "JobMgt.aspx?type=" + GetCareerType();
            btnBack.Attributes["onclick"] = "javascript:$find('PopUp').show(); window.navigate('@backurl'); return false;".Replace("@backurl",backurl);

            Dictionary<Type, string> QueryList = new Dictionary<Type, string>();
            string cond = "acid=" + Request["cid"];
            QueryList.Add(typeof(Application), cond);
            ViewState["Query"] = QueryList;
            ObjGrid.FillGrid(GetQuery(true));
    ...
    private string GetCareerType()
    {
        ResumeInfoDataContext db = new ResumeInfoDataContext();
        IQueryable<Career> careers = db.Career.Where(o => o.cid == int.Parse(Request["cid"]));
        int type = 1;
        foreach (Career carrer in careers)
        {
            type = carrer.ctype;
        }
